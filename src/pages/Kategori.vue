<template>
    <div class="surface-section px-4 py-8 md:px-6 lg:px-8">
        <div class="font-bold text-900 text-3xl mb-3 text-center">Postingan dengan kategori {{ kategori }}</div>
        <DataView :value="blogs" :layout="layout" paginatorPosition="both" :paginator="true" :rows="10" :lazy="true"
            v-if="blogs.length > 0">
    ...
    export default {
        ...
        beforeCreate() {
            axios.get('/sanctum/csrf-cookie').then(() => {
                axios.get('/api/app/blog_kategori/' + this.$route.params.nama).then((result) => {
