            ...
            <div class="card">
                ...
                <DataTable :value="allData" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <template #header>
                        <div class="table-header flex flex-column md:flex-row md:justify-content-between">
                            <h5 class="mb-2 md:m-0 p-as-md-center">Data program studi</h5>
                    ...
                    <Column field="deskripsi" header="Deskripsi" :sortable="true"></Column>
                ...
                <Dialog v-model:visible="modal" :style="{width: '500px'}" header="Popup form" :modal="true"
                    ...
                    <div class="field">
                        ...
                        <Textarea placeholder="Deskripsi singkat" v-model.trim="firstData.deskripsi" :autoResize="true"
                            rows="3" cols="30" :class="{'p-invalid': err.deskripsi}" />
                        <small class=" p-invalid" v-if="err.deskripsi">{{ err.deskripsi[0] }}</small>
                    </div>
                    <div class="field">
                ...
                <Dialog v-model:visible="modalDelete" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="firstData">Anda yakin ingin menghapus data program studi dengan nama
    ...
    export default {
        ...
        created() {
         ...
         beforeCreate() {
            axios.get('/sanctum/csrf-cookie').then(() => {
                axios.get('/api/admin/program_studi', {
                    headers: {
        ...
        methods: {
            ...
            save() {
                ...
                axios.get('/sanctum/csrf-cookie').then(() => {
                    if (this.insert) {
                        axios.post('/api/admin/program_studi', form, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
                        ...
                        axios.post('/api/admin/program_studi/' + this.firstData.id, form, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
            ...
            deleteData() {
                this.disable = true
                axios.get('/sanctum/csrf-cookie').then(() => {
                    axios.delete('/api/admin/program_studi/' + this.firstData.id, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                        }
                    }).then(() => {
