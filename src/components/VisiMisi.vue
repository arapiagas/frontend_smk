        ...
        <div class="col-12">
            <div class="card">
                ...
                <DataTable :value="allData" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <template #header>
                        <div class="table-header flex flex-column md:flex-row md:justify-content-between">
                            <h5 class="mb-2 md:m-0 p-as-md-center">Data visi misi</h5>
                    ...
                    <Column field="isi_text" header="Isi text" :sortable="true"></Column>
                    <Column field="tipe" header="Tipe" :sortable="true"></Column>
                ...
                <Dialog v-model:visible="modal" :style="{width: '500px'}" header="Popup form" :modal="true"
                    ...
                    <div class="field">
                        <label class="mb-3">Tipe</label>
                        <div class="formgrid grid">
                            <div class="field-radiobutton col-6">
                                <RadioButton id="tipe1" name="tipe" value="visi" v-model="firstData.tipe" />
                                <label for="tipe1">Visi</label>
                            </div>
                            <div class="field-radiobutton col-6">
                                <RadioButton id="tipe2" name="tipe" value="misi" v-model="firstData.tipe" />
                                <label for="tipe2">Misi</label>
                            </div>
                        </div>
                        <small class="p-invalid" v-if="err.tipe">{{ err.tipe[0] }}</small>
                    </div>
                    <template #footer>
                ...
                <Dialog v-model:visible="modalDelete" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="firstData">Anda yakin ingin menghapus satu data <b>{{firstData.tipe}}</b>?</span>
                    </div>
                    <template #footer>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                axios.get('/sanctum/csrf-cookie').then(() => {
                    if (this.insert) {
                        axios.post('/api/admin/visi_misi', this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
                        ...
                        axios.put('/api/admin/visi_misi/' + this.firstData.id, this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
            ...
            deleteData() {
                this.disable = true
                axios.get('/sanctum/csrf-cookie').then(() => {
                    axios.delete('/api/admin/visi_misi/' + this.firstData.id, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                        }
                    }).then(() => {
