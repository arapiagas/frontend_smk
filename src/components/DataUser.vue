            ...
            <div class="card">
                ...
                <DataTable :value="allData" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <template #header>
                        <div class="table-header flex flex-column md:flex-row md:justify-content-between">
                            <h5 class="mb-2 md:m-0 p-as-md-center">Data user</h5>
                ...
                <Dialog v-model:visible="modal" :style="{width: '500px'}" header="Popup form" :modal="true"
                    ...
                    <div class="field">
                        ...
                        <InputText id="email" v-model.trim="firstData.email" required="true"
                            :class="{'p-invalid': err.email}" />
                        <small class="p-invalid" v-if="err.email">{{ err.email[0] }}</small>
                    </div>
                    <div class="field">
                        <label for="level">Level</label>
                        <Dropdown v-model="firstData.level" :options="dropData" optionValue="value" optionLabel="name"
                            placeholder="Pilih level user" />
                        <small class="p-invalid" v-if="err.level">{{ err.level[0] }}</small>
                    </div>
                    <div class="field">
                        <label for="password">Password</label>
                        <Password id="password" v-model="firstData.password" placeholder="Password" required="true"
                            :class="{'p-invalid': err.password}"></Password>
                        <small class="p-invalid" v-if="err.password">{{ err.password[0] }}</small>
                    </div>
                    <template #footer>
                ...
                <Dialog v-model:visible="modalDelete" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="firstData">Anda yakin ingin menghapus data user dengan email
                            <b>{{firstData.email}}</b>?</span>
                    </div>
                    <template #footer>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                axios.get('/sanctum/csrf-cookie').then(() => {
                    if (this.insert) {
                        axios.post('/api/admin/user', this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
                        ...
                        axios.put('/api/admin/user/' + this.firstData.id, this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
            ...
            deleteData() {
                axios.get('/sanctum/csrf-cookie').then(() => {
                    axios.delete('/api/admin/user/' + this.firstData.id, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                        }
                    }).then(() => {
