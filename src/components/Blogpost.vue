            ...
            <div class="card">
                ...
                <DataTable :value="allData" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <template #header>
                        <div class="table-header flex flex-column md:flex-row md:justify-content-between">
                            <h5 class="mb-2 md:m-0 p-as-md-center">Data postingan</h5>
                    ...
                    <Column field="kategori.nama" header="Kategori" :sortable="true"></Column>
                ...
                <Dialog v-model:visible="modal" :style="{width: '500px'}" header="Popup form" :modal="true"
                    ...
                    <div class="field">
                        ...
                        <InputText id="judul" v-model.trim="firstData.judul" required="true" autofocus
                            :class="{'p-invalid': err.judul}" />
                        <small class="p-invalid" v-if="err.judul">{{ err.judul[0] }}</small>
                    </div>
                    <div class="field">
                        <label for="kategori">Kategori</label>
                        <Dropdown v-model="firstData.kategori_id" :options="dropData" optionValue="value"
                            optionLabel="name" placeholder="Pilih kategori user" :filter="true"
                            filterPlaceholder="Cari...">
                        </Dropdown>
                        <small class="p-invalid" v-if="err.kategori_id">{{ err.kategori_id[0] }}</small>
                    </div>
                    <div class="field">
                        <label for="isi_text">Isi text</label>
                        <Textarea placeholder="Isi text" v-model.trim="firstData.isi_text" :autoResize="true" rows="3"
                            cols="30" :class="{'p-invalid': err.isi_text}" />
                ...
                <Dialog v-model:visible="modalDelete" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="firstData">Anda yakin ingin menghapus data blogpost dengan judul
                            <b>{{firstData.judul}}</b>?</span>
                    </div>
                    <template #footer>
    ...
    export default {
            ...
            axios.get('/sanctum/csrf-cookie').then(() => {
                axios.get('/api/admin/kategori', {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        this.dropData.unshift(kategori)
                    }
                    console.log(data)
                }).catch((err) => {
                    console.log(err.response.data)
        ...
        methods: {
            ...
            save() {
                ...
                axios.get('/sanctum/csrf-cookie').then(() => {
                    if (this.insert) {
                        axios.post('/api/admin/blog', form, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
                        ...
                        axios.post('/api/admin/blog/' + this.firstData.id, form, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
            ...
            deleteData() {
                this.disable = true
                axios.get('/sanctum/csrf-cookie').then(() => {
                    axios.delete('/api/admin/blog/' + this.firstData.id, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                        }
                    }).then(() => {
