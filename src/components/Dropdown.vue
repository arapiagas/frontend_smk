            ...
            <div class="card">
                ...
                <DataTable :value="allData" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <template #header>
                        <div class="table-header flex flex-column md:flex-row md:justify-content-between">
                            <h5 class="mb-2 md:m-0 p-as-md-center">Data dropdown menu</h5>
                    ...
                    <Column field="menu.title" header="Menu" :sortable="true"></Column>
                    <Column field="title" header="Title" :sortable="true"></Column>
                    <Column field="link" header="Link" :sortable="true"></Column>
                    <Column field="aktif" header="Aktif Y/N" :sortable="true"></Column>
                ...
                <Dialog v-model:visible="modal" :style="{width: '500px'}" header="Popup form" :modal="true"
                    ...
                    <div class="field">
                        ...
                        <InputText placeholder="isi # jika tidak ingin mengarah ke halaman tertentu"
                            v-model.trim="firstData.link" required="true" autofocus :class="{'p-invalid': err.link}" />
                        <small class="">awali dengan tanda <b>/</b> untuk mengisi link</small>
                        <small class=" p-invalid" v-if="err.link">{{ err.link[0] }}</small>
                    </div>
                    <div class="field">
                        <label for="menu_id">Menu id</label>
                        <Dropdown v-model="firstData.menu_id" :options="dropData" optionValue="value" optionLabel="name"
                            placeholder="Pilih menu_id user" />
                        <small class="p-invalid" v-if="err.menu_id">{{ err.menu_id[0] }}</small>
                    </div>
                    <div class="field">
                ...
                <Dialog v-model:visible="modalDelete" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="firstData">Anda yakin ingin menghapus data menu dropdown dengan title
    ...
    export default {
        ...
        beforeCreate() {
            axios.get('/sanctum/csrf-cookie').then(() => {
                axios.get('/api/admin/menu/dropdown', {
                    headers: {
        ...
        methods: {
            ...
            save() {
                ...
                axios.get('/sanctum/csrf-cookie').then(() => {
                    if (this.insert) {
                        axios.post('/api/admin/menu/dropdown', this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
                        ...
                        axios.put('/api/admin/menu/dropdown/' + this.firstData.id, this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
            ...
            deleteData() {
                this.disable = true
                axios.get('/sanctum/csrf-cookie').then(() => {
                    axios.delete('/api/admin/menu/dropdown/' + this.firstData.id, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                        }
                    }).then(() => {
