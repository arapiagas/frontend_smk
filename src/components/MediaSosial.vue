            ...
            <div class="card">
                ...
                <DataTable :value="allData" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <template #header>
                        <div class="table-header flex flex-column md:flex-row md:justify-content-between">
                            <h5 class="mb-2 md:m-0 p-as-md-center">Data media sosial</h5>
                    ...
                    <Column field="ikon" header="Ikon" :sortable="true">
                        <template #body="slotProps">
                            <i :class="'pi ' + slotProps.data.ikon"></i>
                            <div> {{ slotProps.data.ikon }} </div>
                        </template>
                    </Column>
                ...
                <Dialog v-model:visible="modal" :style="{width: '500px'}" header="Popup form" :modal="true"
                    ...
                    <div class="field">
                        ...
                        <InputText id="link" v-model.trim="firstData.link" required="true" autofocus
                            :class="{'p-invalid': err.link}" />
                        <small class="p-invalid" v-if="err.link">{{ err.link[0] }}</small>
                    </div>
                    <div class="field">
                        <label for="ikon">Ikon</label>
                        <InputText id="ikon" placeholder="Masukkan nama ikon" v-model.trim="firstData.ikon"
                            required="true" autofocus :class="{'p-invalid': err.ikon}" />
                        <small>Nama ikon bisa didapat dalam menu <router-link to="/pendidik/daftar_ikon">List ikon
                            </router-link> </small>
                        <small class="p-invalid" v-if="err.ikon">{{ err.ikon[0] }}</small>
                    </div>
                    <template #footer>
                ...
                <Dialog v-model:visible="modalDelete" :style="{width: '450px'}" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="firstData">Anda yakin ingin menghapus data media sosial dengan nama
    ...
    export default {
        ...
        beforeCreate() {
            axios.get('/sanctum/csrf-cookie').then(() => {
                axios.get('/api/admin/media_sosial', {
                    headers: {
        ...
        methods: {
            ...
            save() {
                ...
                axios.get('/sanctum/csrf-cookie').then(() => {
                    if (this.insert) {
                        axios.post('/api/admin/media_sosial', this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
                        ...
                        axios.put('/api/admin/media_sosial/' + this.firstData.id, this.firstData, {
                            headers: {
                                Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                            }
                        }).then((result) => {
            ...
            deleteData() {
                axios.get('/sanctum/csrf-cookie').then(() => {
                    axios.delete('/api/admin/media_sosial/' + this.firstData.id, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('WebToken')
                        }
                    }).then(() => {
